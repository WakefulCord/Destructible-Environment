...
public class Cubes : MonoBehaviour
{
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int z = 0; z < width; z++)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        Vector3Int corner = new Vector3Int(x, y, z) + marching.Corners[i];
    ...
    private void MarchCube(Vector3 position, float[] cubeCorners)
    {
        ...
        for (int t = 0; t < 5; t++)
        {
            for (int v = 0; v < 3; v++)
            {
                int triTableValue = marching.Triangles[configIndex, edgeIndex];
                ...
                Vector3 edgeStart = position + marching.Edges[triTableValue, 0];
                Vector3 edgeEnd = position + marching.Edges[triTableValue, 1];
